...
export function getQuizAvailability(quiz: {
  ...
  code: "active" | "scheduled" | "expired" | "closed";
  label: string;
  isAvailable: boolean;
  reason: string;
  ...
  if (!quiz) {
    return { code: "closed", label: "غير متاح", isAvailable: false, reason: "بيانات الاختبار غير متاحة" };
  }
  ...
  if (quiz.status === "closed") {
    return {
      ...
      label: "مغلق",
      isAvailable: false,
      reason: "الاختبار مغلق حالياً من قبل المعلم",
    };
  }

  const now = Date.now();

  if (quiz.availabilityStart) {
    const startTime = new Date(quiz.availabilityStart).getTime();
    if (!isNaN(startTime) && now < startTime) {
      const formattedStart = new Date(quiz.availabilityStart).toLocaleString("ar-SA", {
        year: "numeric",
        month: "short",
        day: "numeric",
        hour: "2-digit",
      ...
      return {
        ...
        label: "مجدول",
        isAvailable: false,
        reason: `الاختبار مجدول وسيكون متاحاً للتقديم بدءاً من: ${formattedStart}`,
        formattedStart,
      };
    }
  }
  ...
  if (quiz.availabilityEnd) {
    const endTime = new Date(quiz.availabilityEnd).getTime();
    if (!isNaN(endTime) && now > endTime) {
      const formattedEnd = new Date(quiz.availabilityEnd).toLocaleString("ar-SA", {
        year: "numeric",
        month: "short",
        day: "numeric",
        hour: "2-digit",
      ...
      return {
        ...
        label: "منتهي",
        isAvailable: false,
        reason: `انتهت فترة إتاحة هذا الاختبار في: ${formattedEnd}`,
        formattedEnd,
      };
    }
  }
...
export default function App() {
  ...
  const [studentLoggedInId, setStudentLoggedInId] = useState<string | null>(
    () => {
      return sessionStorage.getItem("seb_student_logged_id");
    },
  );
  ...
  // Synchronize student session ID to sessionStorage (tab-scoped) and purge legacy localStorage
  useEffect(() => {
    if (studentSelectedId) {
      sessionStorage.setItem("seb_student_logged_id", studentSelectedId);
      setStudentLoggedInId(studentSelectedId);
    } else {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      ...
      if (user) {
        ...
        sessionStorage.removeItem("seb_student_logged_id"); localStorage.removeItem("seb_student_logged_id");
        ...
        if (targetQuizId && targetQuizId !== "portal") {
          try {
            ...
            if (loadedQuiz) {
              ...
              const avail = getQuizAvailability(qData);
              if (!avail.isAvailable) {
                setStudentQuizError(avail.reason);
                setStudentQuizLoading(false);
                return;
              }
  ...
  const handleStudentSubmitQuiz = async (isAutoSubmit = false) => {
    ...
    try {
      ...
      if (studentNewToggle || !studentSelectedId) {
        ...
        setStudentNewToggle(false);
        sessionStorage.setItem("seb_student_logged_id", targetStudentId);
      } else {
  ...
  useEffect(() => {
    ...
    const activeQuizCount = quizzes.filter((q) => getQuizAvailability(q).code === "active").length;
  ...
  const [builderPdfMcqCount, setBuilderPdfMcqCount] = useState<number>(5);
  ...
  const handleAutoCreateQuiz = async (
    ...
    // Create title based on metadata without the word "اختبار"
    const parts: string[] = [];
    if (meta.subject && meta.subject !== "all" && meta.subject.trim() !== "") {
      parts.push(meta.subject.trim());
    }
    if (meta.unit && meta.unit !== "all" && meta.unit.trim() !== "") {
      parts.push(meta.unit.trim());
    }
    if (meta.lesson && meta.lesson !== "all" && meta.lesson.trim() !== "" && meta.lesson !== "جميع الدروس") {
      parts.push(meta.lesson.trim());
    } else if (meta.unit && meta.unit !== "all" && meta.unit.trim() !== "") {
      parts.push("جميع الدروس");
    }
    
    const quizTitle = parts.length > 0 ? parts.join(" - ") : "تقييم عام";
    const subject = meta.subject !== "all" ? meta.subject : "";
    const grade = meta.grade !== "all" ? meta.grade : "";
    const semester = meta.semester !== "all" ? meta.semester : "";
    ...
    try {
      ...
      triggerToast("حدث خطأ أثناء حفظ الاختبار التلقائي تلقائياً", "error");
    }
  };
  ...
  const handleConfirmBuilderAutoQuiz = async () => {
    ...
    // Construct title based on filters without the word "اختبار"
    const parts: string[] = [];
    if (importFilterSubject && importFilterSubject !== "all" && importFilterSubject.trim() !== "") {
      parts.push(importFilterSubject.trim());
    }
    if (importFilterUnit && importFilterUnit !== "all" && importFilterUnit.trim() !== "") {
      parts.push(importFilterUnit.trim());
    }

    if (importFilterLessons.length > 0) {
      const contextQuestions = bankQuestions.filter(
        (q) =>
          (importFilterStage === "all" || q.stage === importFilterStage) &&
          (importFilterGrade === "all" || q.grade === importFilterGrade) &&
          (importFilterSubject === "all" || q.subject === importFilterSubject) &&
          (importFilterSemester === "all" || q.semester === importFilterSemester)
      );

      const unitLessonsMap = new Map<string, Set<string>>();
      contextQuestions.forEach((q) => {
        if (q.unit && q.lesson) {
          const u = q.unit.trim();
          const l = q.lesson.trim();
          if (u && l) {
            if (!unitLessonsMap.has(u)) unitLessonsMap.set(u, new Set());
            unitLessonsMap.get(u)!.add(l);
          }
        }
      });

      const selectedUnits = new Set<string>();
      importFilterLessons.forEach((key) => {
        const p = key.split(" | ");
        if (p.length === 2) {
          selectedUnits.add(p[0].trim());
        }
      });

      let allSelectedUnitsAreComplete = selectedUnits.size > 0;
      const lessonDisplayNames: string[] = [];

      selectedUnits.forEach((u) => {
        const availableInUnit = unitLessonsMap.get(u) ? Array.from(unitLessonsMap.get(u)!) : [];
        const selectedInUnit = availableInUnit.filter((l) =>
          importFilterLessons.includes(`${u} | ${l}`)
        );

        if (availableInUnit.length > 0 && selectedInUnit.length === availableInUnit.length) {
          // All lessons in this unit are selected
        } else {
          allSelectedUnitsAreComplete = false;
          selectedInUnit.forEach((l) => lessonDisplayNames.push(l));
        }
      });

      // Make sure unit names from selected lessons are added if not already present
      selectedUnits.forEach((uName) => {
        if (uName && !parts.includes(uName)) {
          parts.push(uName);
        }
      });

      if (allSelectedUnitsAreComplete) {
        parts.push("جميع الدروس");
      } else if (lessonDisplayNames.length > 0) {
        parts.push(lessonDisplayNames.join(", "));
      } else {
        parts.push(importFilterLessons.map((k) => k.split(" | ")[1] || k).join(", "));
      }
    }
    ...
    try {
      ...
      try {
        ...
        if (res.status === 504 || res.status === 502) {
          throw new Error("استغرقت المعالجة وقتاً طويلاً على الخادم (مهلة الاتصال). يرجى تقليل عدد الأسئلة أو اختيار دروس محددة والمحاولة مجدداً.");
        }
        if (!res.ok) {
          throw new Error(`خطأ من الخادم (رمز الحالة: ${res.status}). يرجى التحقق من اتصال الشبكة وإعادة المحاولة.`);
        }
        throw new Error("تلقى التطبيق استجابة غير مكتملة من الخادم. يرجى تجربة اختيار عدد أقل من الأسئلة أو إعادة المحاولة.");
      }
    ...
    const mapped: Question[] = draftsToSave.map((draft, idx) => {
      ...
      if (draftType === 'true_false') {
        ...
        if (/^[0-3]$/.test(rawCorrect)) {
          const cIdx = parseInt(rawCorrect, 10);
          if (cIdx >= 0 && cIdx < rawDraftOpts.length) {
            correctOpt = rawDraftOpts[cIdx];
          }
        } else {
  ...
  const handleSaveQuiz = async (e: React.FormEvent) => {
    ...
    const newQuiz: Quiz = {
      ...
      status: existingQuiz ? existingQuiz.status : "closed",
    ...
    try {
      ...
      const successMsg = editingQuizId
        ? "تم تحديث التعديلات بنجاح!"
        : "تم حفظ الاختبار كمسودة مغلقة بنجاح! يمكنك تفعيله ونشره من لوحة التحكم متى أردت.";
  ...
  const builderTeacherGrades = useMemo(() => {
    ...
    return fromBank;
  }, [gradesList, bankQuestions]);

  const builderTeacherSemesters = useMemo(() => {
    if (builderTargetGrade !== "all") {
      const sems = getSemestersForGrade(builderTargetGrade);
  ...
  }, [builderTargetGrade, getSemestersForGrade, semesters]);
  ...
  // Reset/Delete quiz score for specified student IDs so they can retake the quiz
  const handleResetQuizForStudents = async (
    studentIds: string[],
    quizTitle: string
  ) => {
    ...
    await runWithProgress(
      async () => {
        const studentUpdates = studentIds.map(async (sId) => {
          const targetStudentObj = students.find((s) => s.id === sId);
          if (!targetStudentObj) return;

          const gradesArray = targetStudentObj.detailedGrades || [];
          const updatedGrades = gradesArray.filter(
            (g) => g.quizTitle !== quizTitle
          );
          ...
          // Update selectedManageStudent if relevant
      ...
      "جاري حذف نتيجة الاختبار وتحديث السجلات...",
      studentIds.length === 1
  ...
  const filteredQuizzes = quizzes.filter((q) => {
    ...
    const matchesFilter =
      quizFilter === "all" ||
      (quizFilter === "active" && q.status === "active" && avail.code !== "expired") ||
      (quizFilter === "closed" && (q.status === "closed" || avail.code === "expired"));
        ...
        >
          ...
          >
            ...
            <div className="space-y-3">
              ...
              >
                <span>العودة لمنصة معلم | Teacher.AI الرئيسية</span>
              </button>
            </div>
  ...
  if (
            ...
            <div className="p-6 md:p-8 space-y-6 bg-slate-50/50">
              <div className="p-5 rounded-2xl bg-white border border-indigo-100 shadow-sm text-right space-y-4">
                <div className="flex items-start gap-3.5">
                  ...
                  <div>
                    <h2 className="text-base font-black text-indigo-950 whitespace-nowrap">
                      معلم | Teacher.AI
                    </h2>
                ...
                <div className="pt-2 border-t border-indigo-100/60 flex items-center justify-center gap-2 text-center text-indigo-900 font-bold text-sm bg-indigo-50/70 p-3 rounded-xl border border-indigo-100 whitespace-nowrap">
                  <Sparkles className="w-4 h-4 text-amber-500 shrink-0" />
                  <span>معلم: بوابتك الذكية لعالم Teacher.AI</span>
                </div>
              </div>
    ...
    if (!studentSelectedId) {
      ...
      const handleStudentPortalLogin = async (e?: React.FormEvent) => {
        ...
        if (match) {
          ...
          sessionStorage.setItem("seb_student_logged_id", match.id);
          ...
          >
            <div className="p-8 md:p-10 bg-gradient-to-tr from-indigo-750 to-indigo-600 text-white text-center relative font-sans">
              ...
              <div className="mt-1.5 flex justify-center">
                <span className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-amber-300 text-indigo-950 text-xs font-black shadow-md border border-amber-200 tracking-wide whitespace-nowrap">
                  <Sparkles className="w-3.5 h-3.5 text-indigo-950 shrink-0" />
            ...
            <form onSubmit={handleStudentPortalLogin} className="p-8 md:p-10 space-y-6 flex-1 flex flex-col justify-between">
              ...
              <div className="pt-4 mt-2 border-t border-slate-100 text-center">
                <p className="text-[11px] font-bold text-slate-400">
                  جميع الحقوق محفوظة © {new Date().getFullYear()} معلم | Teacher.AI
                </p>
              </div>
    ...
    if (!studentQuizStarted) {
      ...
      const renderStudentTabContent = (navKey: string) => {
        switch (navKey) {
          case "home":
            return (() => {
              ...
              return (
                <div className="space-y-6 animate-fadeIn">
                  {/* Hero Welcome banner */}
                  <div className="bg-gradient-to-r from-indigo-900 via-indigo-800 to-slate-900 p-6 rounded-3xl text-white shadow-xl relative overflow-hidden border border-indigo-700/50">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                      <div>
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 border border-indigo-400/30 text-indigo-200 text-xs font-extrabold mb-3">
                          <Sparkles className="w-3.5 h-3.5 text-amber-400" />
                          <span>أهلاً بك مجدداً في منصتك الدراسية</span>
                        </div>
                        <h2 className="text-2xl md:text-3xl font-black tracking-tight">
                          مرحباً بك يا بطل، {activeStudent.name}! 👋
                        </h2>
                        <p className="text-slate-300 text-xs md:text-sm mt-1 max-w-xl font-medium leading-relaxed">
                          تابع تقدمك الدراسي، أنجز اختباراتك المدرسية المتاحة، وشارك في التحديات والمراجعة الشاملة للمناهج.
                        </p>
                      </div>
                      ...
                      <div className="flex items-center gap-3 bg-white/10 backdrop-blur-md px-4 py-3 rounded-2xl border border-white/10 shrink-0">
                        ...
                        <div>
                          <span className="text-[10px] text-slate-300 font-bold block">معدلك العام</span>
                          <span className="text-xl font-black text-white font-sans">{activeStudent.averageScore}%</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                    <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex items-center gap-3">
                      ...
                      <div>
                        <span className="text-[10px] text-slate-400 font-bold block">اختبارات قادمة</span>
                        <span className="text-base font-black text-slate-800 font-sans">{pendingQuizzesCount}</span>
                      </div>
                    </div>
                    ...
                    <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex items-center gap-3">
                      ...
                      <div>
                        <span className="text-[10px] text-slate-400 font-bold block">اختبارات مكتملة</span>
                        <span className="text-base font-black text-slate-800 font-sans">{completedQuizzesCount}</span>
                      </div>
                    </div>
                    ...
                    <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex items-center gap-3">
                      ...
                      <div>
                        <span className="text-[10px] text-slate-400 font-bold block">أعلى نتيج ألعاب</span>
                        <span className="text-base font-black text-slate-800 font-sans">{highestGameScore}</span>
                      </div>
                    </div>
                    ...
                    <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex items-center gap-3">
                      ...
                      <div>
                        <span className="text-[10px] text-slate-400 font-bold block">دروس مراجعة محلولة</span>
                        <span className="text-base font-black text-slate-800 font-sans">{solvedCount}</span>
                      </div>
                    </div>
                  </div>

                  {/* Direct portal links & shortcut buttons */}
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div
                      onClick={() => setStudentActiveNav("quizzes")}
                      className="bg-white p-5 rounded-2xl border border-slate-200 hover:border-indigo-300 shadow-xs hover:shadow-md transition-all cursor-pointer group flex flex-col justify-between space-y-4"
                    >
                      <div className="flex items-center justify-between">
                        <div className="w-10 h-10 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold">
                          <BookOpen className="w-5 h-5" />
                        </div>
                        <span className="text-xs font-black text-indigo-600 group-hover:translate-x-[-2px] transition-transform">دخول &larr;</span>
                      </div>
                      <div>
                        <h3 className="font-extrabold text-slate-900 text-sm">الاختبارات المدرسية المتاحة</h3>
                        <p className="text-xs text-slate-500 mt-1">تصفح وقدم الاختبارات المخصصة لصفك الدراسي ورصد نتائجك بشكل مباشر.</p>
                      </div>
                    </div>
                    ...
                    <div
                      onClick={() => setStudentActiveNav("reviews")}
                      className="bg-white p-5 rounded-2xl border border-slate-200 hover:border-emerald-300 shadow-xs hover:shadow-md transition-all cursor-pointer group flex flex-col justify-between space-y-4"
                    >
                      <div className="flex items-center justify-between">
                        <div className="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center font-bold">
                          <Gamepad2 className="w-5 h-5" />
                        </div>
                        <span className="text-xs font-black text-emerald-600 group-hover:translate-x-[-2px] transition-transform">دخول &larr;</span>
                      </div>
                      <div>
                        <h3 className="font-extrabold text-slate-900 text-sm">الألعاب والتحديات التنافسية</h3>
                        <p className="text-xs text-slate-500 mt-1">نافس زملاءك واكسب النقاط من خلال الألعاب التعليمية التفاعلية الممتعة.</p>
                      </div>
                    </div>
                  </div>

                  {/* Profile info footer note */}
                  <div className="bg-slate-100/80 p-4 rounded-2xl border border-slate-200 text-slate-600 flex items-center justify-between">
                    <div className="flex items-center gap-2 text-xs font-bold">
                      <UserCheck className="w-4 h-4 text-indigo-600" />
                      <span>الصف والفصل: {activeStudent.gradeClass || "غير محدد"}</span>
                    </div>
              ...
              <div className="space-y-6">
                ...
                {selectedCompletedQuizData && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className="bg-indigo-50/70 border border-indigo-150 p-6 rounded-3xl shadow-sm space-y-4 text-right"
                  >
                    <div className="flex justify-between items-center pb-3 border-b border-indigo-100">
                      <div className="flex items-center gap-2">
                        <Award className="w-5 h-5 text-indigo-600" />
                        <h3 className="font-extrabold text-slate-900 text-base">
                          تفاصيل إجاباتك ورأي المعلم: {selectedCompletedQuizData.quizTitle || selectedCompletedQuizData.title || "اختبار مدرسي"}
                        </h3>
                      </div>
                      <button
                        onClick={() => setSelectedCompletedQuizData(null)}
                        className="text-slate-400 hover:text-slate-600 text-xs font-bold px-3 py-1 rounded-lg bg-white border border-slate-200 cursor-pointer"
                      >
                        إغلاق التقرير ✕
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
                      <div className="bg-white p-3 rounded-2xl border border-indigo-100 shadow-3xs">
                        <span className="text-[10px] text-slate-400 font-bold block">الدرجة النهائية</span>
                        <span className="text-lg font-black text-indigo-700 font-sans">{selectedCompletedQuizData.score || 0} / {selectedCompletedQuizData.maxScore || 100}</span>
                      </div>
                      <div className="bg-white p-3 rounded-2xl border border-indigo-100 shadow-3xs">
                        <span className="text-[10px] text-slate-400 font-bold block font-sans">النسبة المئوية</span>
                        <span className="text-lg font-black text-emerald-600 font-sans">{selectedCompletedQuizData.percentage || Math.round(((selectedCompletedQuizData.score || 0) / (selectedCompletedQuizData.maxScore || 100)) * 100)}%</span>
                      </div>
                      <div className="bg-white p-3 rounded-2xl border border-indigo-100 shadow-3xs">
                        <span className="text-[10px] text-slate-400 font-bold block">التقدير المكتسب</span>
                        <span className="text-xs font-extrabold text-slate-800">{getStatusArabicLabel(selectedCompletedQuizData.status || "good")}</span>
                      </div>
                      <div className="bg-white p-3 rounded-2xl border border-indigo-100 shadow-3xs">
                        <span className="text-[10px] text-slate-400 font-bold block">تاريخ التسليم</span>
                        <span className="text-xs font-bold text-slate-600 font-sans">{selectedCompletedQuizData.submittedAt ? new Date(selectedCompletedQuizData.submittedAt).toLocaleDateString("ar-SA") : "سابقاً"}</span>
                      </div>
                    </div>
                  </motion.div>
                )}

                {/* Available Active Quizzes list */}
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-base md:text-lg font-black text-slate-900 flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-indigo-600" />
                      <span>الاختبارات المتاحة للتقديم ({activeQuizzes.length})</span>
                    </h3>
                  </div>
                  ...
                  {activeQuizzes.length === 0 ? (
                    <div className="p-8 text-center bg-white rounded-3xl border border-slate-200/80 space-y-2">
                      ...
                      <h4 className="font-extrabold text-slate-800 text-sm">ممتاز! لا يوجد اختبارات معلقة حالياً 🎉</h4>
                      <p className="text-xs text-slate-500 max-w-md mx-auto">
                        لقد قمت بإنجاز جميع الاختبارات المتاحة الموجهة لصفك الدراسي. يمكنك مراجعة نتائجك السابقة أو المشاركة في المراجعة الشاملة.
                      </p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {activeQuizzes.map((quiz) => {
                        ...
                        return (
                          <div
                            key={quiz.id}
                            className="bg-white p-5 rounded-2xl border border-slate-200/90 hover:border-indigo-300 shadow-3xs hover:shadow-md transition-all flex flex-col justify-between space-y-4 text-right"
                          >
                            <div className="space-y-2">
                              <div className="flex items-center justify-between gap-2">
                                <span className="px-2.5 py-1 bg-indigo-50 text-indigo-700 text-[10px] font-black rounded-lg border border-indigo-100">
                                  {quiz.subject || "اختبار مدرسي"}
                                </span>
                                {avail.code === "scheduled" ? (
                                  <span className="px-2 py-0.5 bg-amber-50 text-amber-800 border border-amber-200/80 text-[10px] font-extrabold rounded-md flex items-center gap-1">
                              ...
                              <p className="text-xs text-slate-500 line-clamp-2">اختبار تقويمي تفاعلي لقياس المفاهيم والمهارات الدراسية.</p>
                            </div>
                            ...
                            {avail.code === "scheduled" ? (
                              <div className="bg-amber-50/80 border border-amber-200 rounded-xl p-3 text-center space-y-1">
                                <span className="text-xs font-black text-amber-900 flex items-center justify-center gap-1.5">
                                  ...
                                  <span>اختبار مجدول وقادم</span>
                                </span>
                                <span className="text-[10px] text-amber-800 font-extrabold block">
                                  متاح للتقديم بدءاً من: {avail.formattedStart}
                                </span>
                              </div>
                            ) : (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  setStudentQuizFinished(false);
                                  triggerToast("تم توجيهك لقاعة تقديم الاختبار بنجاح!", "success");
                                }}
                                className="w-full py-3 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white font-extrabold text-xs rounded-xl shadow-md transition-all duration-200 active:scale-95 flex items-center justify-center gap-2 cursor-pointer"
                              >
                                <span>بدء تقديم الاختبار الآن 🚀</span>
                              </button>
                            )}
                          </div>
                        );
                ...
                {/* Completed Quizzes History list */}
                {completedQuizzes.length > 0 && (
                  <div className="space-y-4 pt-6 border-t border-slate-200">
                    <h3 className="text-base md:text-lg font-black text-slate-900 flex items-center gap-2">
                      <Award className="w-5 h-5 text-amber-500" />
                      <span>أرشيف الاختبارات المكتملة ونتائجك ({completedQuizzes.length})</span>
                    </h3>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {completedQuizzes.map((gradeItem: any, idx: number) => {
                        return (
                          <div
                            key={gradeItem.id || `grade-${idx}`}
                            onClick={() => setSelectedCompletedQuizData(gradeItem)}
                            className="bg-slate-50/80 p-4 rounded-2xl border border-slate-200 hover:bg-white hover:border-indigo-200 shadow-2xs transition-all cursor-pointer flex items-center justify-between gap-3 text-right"
                          >
                            <div className="space-y-1">
                              <h4 className="font-extrabold text-slate-800 text-sm">{gradeItem.quizTitle || gradeItem.title || "اختبار مدرسي"}</h4>
                              <p className="text-[11px] text-slate-500 font-medium">انقر لاستعراض تفاصيل النتيجة والتقدير المكتسب.</p>
                            </div>

                            <div className="text-left shrink-0 bg-white px-3 py-2 rounded-xl border border-slate-200">
                              <span className="text-[10px] text-slate-400 font-bold block">الدرجة</span>
                              <span className="text-sm font-black text-indigo-700 font-sans">{gradeItem.score || 0} / {gradeItem.maxScore || 100}</span>
                            </div>
                          </div>
                        );
                      })}
        ...
        >
          ...
          {showStudentSidebar && (
            ...
            <div className="space-y-6 relative z-10">
              ...
              <div className="flex items-center gap-3 pb-5 border-b border-slate-800">
                ...
                <div>
                  <h1 className="text-sm font-extrabold text-white leading-none leading-tight mb-0.5">
                    بوابة الطالب الإلكترونية
                  </h1>
                  <span className="inline-flex items-center gap-1 my-1 px-2.5 py-0.5 rounded-full bg-amber-300 text-indigo-950 text-[10.5px] font-black leading-tight shadow-sm border border-amber-200 whitespace-nowrap">
                    <Sparkles className="w-2.5 h-2.5 text-indigo-950 shrink-0" />
                    <span>معلم | Teacher.AI</span>
                  </span>
                  <motion.div
          ...
          <div className={`flex-1 min-h-screen ${showStudentSidebar ? "bg-slate-50 hidden md:flex" : "bg-slate-950 flex"} flex-col`}>
            ...
            <main className={`p-4 md:p-8 flex-1 ${studentActiveNav === "curriculum_review" ? "w-full max-w-none" : (showStudentSidebar ? "max-w-6xl" : "max-w-7xl")} w-full mx-auto space-y-8`}>
              ...
              {/* Student Page Footer Copyright */}
              <footer className="mt-12 pt-6 pb-4 border-t border-slate-200/80 text-center text-xs text-slate-500 font-bold flex flex-col sm:flex-row items-center justify-between gap-3">
                <div className="flex items-center gap-2 text-indigo-900 font-black whitespace-nowrap">
                  <Sparkles className="w-4 h-4 text-amber-500 shrink-0" />
                  <span>بوابة الطالب - معلم | Teacher.AI</span>
                </div>
    ...
    return (
      ...
      >
        ...
        <main className="max-w-4xl mx-auto w-full p-4 md:p-6 space-y-6 flex-1">
          ...
          {/* Student Exam Screen Footer Copyright */}
          <footer className="mt-8 pt-4 border-t border-slate-200 text-center text-[11px] text-slate-400 font-bold whitespace-nowrap">
            جميع الحقوق محفوظة © {new Date().getFullYear()} معلم | Teacher.AI
          </footer>
        </main>
      </div>
    ...
    >
      ...
      {!(isCurriculumAdminFullScreen && activeTab === "curriculum_review_admin") && (
        ...
        <div className="space-y-6 relative z-10">
          ...
          <div className="flex flex-col gap-3 pb-4 border-b border-slate-100">
            <div className="flex items-center gap-3">
              <div className="relative w-11 h-11 rounded-2xl bg-gradient-to-tr from-[#1e3a8a] via-indigo-700 to-blue-600 p-0.5 shadow-lg shadow-indigo-600/20 shrink-0 transform hover:scale-105 transition-all duration-200 group cursor-pointer">
                <div className="w-full h-full bg-gradient-to-b from-indigo-950 to-[#0f172a] rounded-[14px] flex items-center justify-center relative overflow-hidden border border-amber-400/40">
                  <div className="absolute inset-0 bg-gradient-to-tr from-amber-500/15 via-transparent to-blue-500/20" />
                  <GraduationCap className="w-5 h-5 text-amber-300 drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)] transform -translate-x-0.5" />
                  <span className="absolute bottom-0.5 right-0.5 bg-gradient-to-r from-amber-400 to-yellow-300 text-indigo-950 font-black text-[8px] px-1 py-0.2 rounded-md shadow-md border border-white/60 leading-none flex items-center justify-center font-sans">
                    AI
                  </span>
                  <Sparkles className="w-2.5 h-2.5 text-amber-300 absolute top-1 left-1 animate-pulse" />
                </div>
              </div>
              <div className="min-w-0">
                <h1 className="text-xs sm:text-sm md:text-base font-extrabold text-slate-900 leading-tight whitespace-nowrap">
                  معلم | Teacher.AI
                </h1>
          ...
          <nav className="space-y-1.5">
            ...
            {/* Separator & Distinct Add Students/Classes Item */}
              ...
              <button
                ...
                className={`w-full flex items-center justify-between gap-2 px-3.5 py-2.5 rounded-xl text-xs font-black transition-all duration-200 transform hover:-translate-y-0.5 hover:scale-105 active:scale-95 cursor-pointer ${
                  activeTab === "students"
                    ...
                    : "bg-emerald-50/90 hover:bg-emerald-100 text-emerald-950 border border-emerald-200/80 font-black shadow-xs"
                }`}
              >
                <div className="flex items-center gap-3">
                  <Users className={`w-4 h-4 shrink-0 ${activeTab === "students" ? "text-white" : "text-emerald-600"}`} />
        ...
        <div className="pt-4 border-t border-slate-100 space-y-3 relative z-10 font-sans">
          ...
          {/* Teacher Sidebar Copyright */}
          <div className="pt-3 mt-2 border-t border-slate-200/60 text-center">
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-x-hidden">
        <main className={`flex-1 flex flex-col ${isCurriculumAdminFullScreen && activeTab === "curriculum_review_admin" ? "p-3 md:p-6 max-w-none w-full" : "p-6 md:p-10 max-w-7xl mx-auto w-full"}`}>
          ...
          <AnimatePresence mode="wait">
                          ...
                          >
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="space-y-0.5">
                                <div className="relative group/title">
                                  <h4 className="text-sm font-black text-slate-900 group-hover:text-[#1e3a8a] transition-colors leading-tight cursor-pointer line-clamp-2 pr-5" title={quiz.title}>
                                    {quiz.title}
                                  </h4>

                                  {/* Large Title Hover Popup */}
                                  <div className="absolute right-0 bottom-full mb-2 opacity-0 invisible group-hover/title:opacity-100 group-hover/title:visible transition-all duration-200 z-[100] w-[280px] sm:w-[420px] md:w-[520px] bg-slate-900/95 text-white p-4 rounded-2xl shadow-2xl border border-slate-700/80 pointer-events-none backdrop-blur-md transform -translate-y-1 group-hover/title:translate-y-0 text-right whitespace-normal break-words">
                                    <div className="text-amber-300 text-xs font-bold mb-1.5 flex items-center gap-1.5 border-b border-slate-800 pb-1.5">
                                      <Sparkles className="w-4 h-4 shrink-0 text-amber-400" />
                                      <span>العنوان الكامل للاختبار:</span>
                                    </div>
                                    <div className="text-lg sm:text-xl md:text-2xl font-black text-white leading-relaxed break-words whitespace-normal">
                                      {quiz.title}
                                    </div>
                                  </div>
                                </div>
                              ...
                              {/* Status Badge */}
                              <div className="pt-0.5 font-sans flex items-center gap-1.5 flex-wrap">
                                {(() => {
                                  const avail = getQuizAvailability(quiz);
                                  if (avail.code === "scheduled") {
                                    return (
                                      <span className="inline-flex items-center gap-1 text-amber-800 font-extrabold text-[10px] bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-lg shadow-xs">
                                        <Clock className="w-3 h-3 text-amber-600" />
                                        مجدول (متاح: {avail.formattedStart})
                                      </span>
                                    );
                                  }
                                  if (avail.code === "expired") {
                                    return (
                                      <span className="inline-flex items-center gap-1 text-rose-800 font-extrabold text-[10px] bg-rose-50 border border-rose-200 px-2 py-0.5 rounded-lg shadow-xs">
                                        <AlertTriangle className="w-3 h-3 text-rose-600" />
                                        منتهي (انتهى: {avail.formattedEnd})
                                      </span>
                                    );
                                  }
                            ...
                            <div className="pt-2 mt-2 border-t border-slate-100 flex items-center">
                              ...
                              <button
                                ...
                                className={`w-full font-bold py-1 px-2 rounded-lg flex items-center justify-center gap-1 text-[10px] transition duration-150 cursor-pointer shadow-3xs active:scale-[0.98] ${
                                  quiz.status === "active"
                                    ? "bg-red-800 hover:bg-red-900 text-white border border-red-900"
                                    : "bg-emerald-800 hover:bg-emerald-900 text-white border border-emerald-900"
                                }`}
                                title={quiz.status === "active" ? "إيقاف الاختبار" : "تفعيل الاختبار"}
                              >
                                {quiz.status === "active" ? (
                                  <>
                                    <Pause className="w-3 h-3 text-white shrink-0" />
                                    <span>إيقاف</span>
                                  </>
                                ) : (
                                  <>
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2.5 flex-1 min-w-0">
                              ...
                              <div className="min-w-0 pr-1.5">
                                <div className="relative group/title">
                                  <h4 className="text-xs font-black text-slate-900 group-hover:text-[#1e3a8a] transition-colors leading-snug truncate cursor-pointer" title={quiz.title}>
                                    {quiz.title}
                                  </h4>
                            ...
                            <div className="flex flex-wrap items-center gap-3 shrink-0 font-sans text-[10px]">
                              ...
                              <div>
                                {(() => {
                                  ...
                                  if (avail.code === "scheduled") {
                                    return (
                                      <span className="inline-flex items-center gap-1 text-amber-800 font-extrabold text-[9px] bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-md shadow-xs">
                                        <Clock className="w-3 h-3 text-amber-600" />
                                        مجدول
                                      </span>
                                    );
                                  ...
                                  if (avail.code === "expired") {
                                    return (
                                      <span className="inline-flex items-center gap-1 text-rose-800 font-extrabold text-[9px] bg-rose-50 border border-rose-200 px-1.5 py-0.5 rounded-md shadow-xs">
                                        <AlertTriangle className="w-3 h-3 text-rose-600" />
                                        منتهي
                                      </span>
                                    );
                            ...
                            <div className="flex flex-col items-start sm:items-end gap-1.5 pr-1.5 shrink-0">
                              ...
                              <div className="flex items-center gap-1.5">
                                ...
                                <button
                                  ...
                                  className={`font-bold py-1 px-2.5 rounded-lg flex items-center justify-center gap-1 text-[10px] transition duration-150 cursor-pointer shadow-3xs active:scale-[0.98] ${
                                    quiz.status === "active"
                                      ? "bg-red-800 hover:bg-red-900 text-white border border-red-900"
                                      : "bg-emerald-800 hover:bg-emerald-900 text-white border border-emerald-900"
                                  }`}
            ...
            {(activeTab === "students" || activeTab === "student_results") &&
              (() => {
                ...
                const triggerExport = async (format: "sheets" | "excel" | "csv" | "pdf" | "print" | "copy") => {
                  if (filteredStudents.length === 0) {
                  ...
                  if (format === "sheets") {
                    ...
                    try {
                      ...
                      link.href = url;
                      link.download = `كشف_درجات_الطلاب_${activeGrade}_${activeSemester}.xlsx`;
                    ...
                    try {
                      ...
                      const link = document.createElement("a");
                      link.href = url;
                      link.download = `كشف_درجات_الطلاب_${activeGrade}_${activeSemester}.csv`;
                      document.body.appendChild(link);
                      link.click();
                      setTimeout(() => {
                        if (document.body.contains(link)) document.body.removeChild(link);
                    ...
                    try {
                      ...
                      triggerToast("📋 تم نسخ درجات جميع الطلاب لجميع الاختبارات! افتح ملف Excel واضغط (Ctrl + V) للصق المباشر.", "success");
                    } catch (error: any) {
                      console.error("Copy error:", error);
                      triggerToast("عفواً، تعذر نسخ الدرجات إلى الحافظة تلقائياً.", "error");
                    }
                  } else if (format === "pdf") {
                    try {
                      triggerToast("جاري إنشاء وتحميل كشف الدرجات كملف PDF...", "info");
                      const currentDate = new Date().toLocaleDateString("ar-SA");
                      const fileName = `كشف_درجات_الطلاب_${activeGrade}_${activeSemester}.pdf`;
                      ...
                      element.innerHTML = `
                        <div style="direction: rtl; font-family: 'Cairo', 'Segoe UI', Tahoma, sans-serif; width: 100%;">
                          <table style="width: 100%; margin-bottom: 20px; border-collapse: collapse;">
                            <tr>
                              <td style="width: 33%; text-align: right; line-height: 1.6; font-size: 11px;">
                                <strong style="font-size: 12px; color: #1e293b;">المملكة العربية السعودية</strong><br>
                                <strong>وزارة التعليم</strong><br>
                                <span style="color: #475569;">منصة رصد الدرجات الذكية</span>
                              </td>
                              <td style="width: 34%; text-align: center; vertical-align: middle;">
                                <h2 style="font-size: 18px; font-weight: 800; color: #1e1b4b; margin: 0 0 5px 0;">كشف رصد درجات الطلاب التراكمي</h2>
                                <div style="font-size: 11px; color: #4338ca; font-weight: bold;">${activeGrade} - ${activeSemester}</div>
                              </td>
                          ...
                          <table style="width: 100%; border-collapse: collapse; font-size: 11px; margin-bottom: 25px;">
                            <thead>
                              <tr style="background-color: #e0e7ff; color: #1e1b4b;">
                                <th style="border: 1px solid #cbd5e1; padding: 10px 6px; text-align: center; vertical-align: middle; line-height: 1.5; width: 35px;">#</th>
                                ${headers.map((h) => `<th style="border: 1px solid #cbd5e1; padding: 10px 8px; text-align: center; vertical-align: middle; line-height: 1.5;">${h}</th>`).join("")}
                              </tr>
                            </thead>
                            <tbody>
                              ${rows
                                ...
                                <tr style="background-color: ${rIdx % 2 === 0 ? "#ffffff" : "#f8fafc"};">
                                  <td style="border: 1px solid #cbd5e1; padding: 8px 6px; text-align: center; vertical-align: middle; line-height: 1.5; font-weight: bold; color: #64748b;">${rIdx + 1}</td>
                                  ${row
                                    .map(
                                      (cell, cIdx) => `
                                    <td style="border: 1px solid #cbd5e1; padding: 8px 8px; text-align: ${cIdx === 0 ? "right" : "center"}; vertical-align: middle; line-height: 1.5; font-weight: ${cIdx === 0 ? "bold" : "normal"};">
                                      ${cell}
                                    </td>
                                  `
                                    )
                          ...
                          <div style="margin-top: 40px; display: flex; justify-content: space-between; padding: 0 30px;">
                            <div style="text-align: center; width: 160px;">
                              <strong style="font-size: 12px; color: #1e293b;">معلم المادة</strong>
                              <div style="margin-top: 35px; border-top: 1px dashed #94a3b8;"></div>
                            </div>
                            <div style="text-align: center; width: 160px;">
                              <strong style="font-size: 12px; color: #1e293b;">المشرف التربوي</strong>
                              <div style="margin-top: 35px; border-top: 1px dashed #94a3b8;"></div>
                            </div>
                            <div style="text-align: center; width: 160px;">
                              <strong style="font-size: 12px; color: #1e293b;">مدير المدرسة</strong>
                              <div style="margin-top: 35px; border-top: 1px dashed #94a3b8;"></div>
                            </div>
                          </div>
                        </div>
                      ...
                      const html2pdfModule: any = (await import("html2pdf.js")).default || (await import("html2pdf.js"));
                      const opt = {
                        margin: [8, 8, 8, 8],
                        filename: fileName,
                        ...
                        html2canvas: {
                          ...
                          onclone: (clonedDoc: Document) => {
                            // 1. Remove all style and link tags to prevent html2canvas from parsing Tailwind 4 CSS rules containing oklab/oklch
                            clonedDoc.querySelectorAll("style, link").forEach((el) => el.remove());

                            // 2. Reset html/body attributes to prevent inheriting CSS variables with unsupported color functions
                            ...
                            // 3. Sanitize inline styles on all cloned elements
                            clonedDoc.querySelectorAll("*").forEach((node) => {
                              const htmlNode = node as HTMLElement;
                              if (htmlNode.style && htmlNode.style.cssText) {
                                if (htmlNode.style.cssText.includes("oklab") || htmlNode.style.cssText.includes("oklch")) {
                                  htmlNode.style.cssText = htmlNode.style.cssText
                      ...
                      triggerToast("🎉 تم تحميل كشف الدرجات كملف PDF بنجاح!", "success");
                    } catch (pdfErr: any) {
                      console.error("PDF generation error:", pdfErr);
                      triggerToast("تعذر إنشاء PDF تلقائياً، جاري فتح معاينة الطباعة...", "info");
                      await triggerExport("print");
                    }
                    ...
                    try {
                      ...
                      // 3. Trigger printing after DOM node insertion
                      ...
                      setTimeout(() => {
                        try {
                          ...
                          console.error("Print call error:", pErr);
                        }
                            ...
                            {activeTab === "student_results" && (
                              <div className="flex flex-wrap items-center gap-2 font-sans">
                                ...
                                <button
                                  ...
                                  title="تحميل كشف الدرجات مباشرة كملف PDF"
                                >
                            ...
                            <table className="w-full text-right border-collapse text-[13px]">
                              <thead>
                                {activeTab === "students" ? (
                                  <tr className="bg-slate-100 text-slate-600 font-bold select-none divide-x divide-x-reverse divide-slate-200">
                                    ...
                                    <th className="py-2 px-3 border border-slate-200 text-center w-52 min-w-[210px] bg-slate-100">
                                      <div className="flex flex-col items-center justify-center gap-2 mx-auto">
                                         <div className="flex items-center justify-center gap-1.5 flex-wrap p-1.5 px-2.5 bg-white border border-slate-300 rounded-lg shadow-2xs w-full max-w-[200px]">
                                           <input
                                             type="checkbox"
                                  ...
                                  <tr className="bg-slate-100 text-slate-600 font-bold select-none divide-x divide-x-reverse divide-slate-200">
                                    <th className="py-2 px-2 border border-slate-200 w-12 text-center font-mono bg-slate-150">
                                      <div className="flex items-center justify-center gap-1" title="تحديد جميع الطلاب">
                                        <input
                                          type="checkbox"
                                          className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer w-3.5 h-3.5"
                                          checked={
                                    ...
                                    <th className="py-2 px-3 border border-slate-200 min-w-[200px]">
                                      اسم الطالب
                                    </th>
                                    {activeClassQuizzes.map((q) => {
                                      const quizMaxScore =
                                      ...
                                      return (
                                        <th
                                          key={q.id}
                                          className="py-2 px-2 border border-slate-200 text-center text-[11px] font-black text-indigo-950 bg-indigo-50/60 min-w-[130px]"
                                        >
                                          <div
                                          ...
                                          <div className="mt-1.5 flex flex-wrap items-center justify-center gap-1">
                                            <button
                                              ...
                                              onClick={(e) => {
                                                e.stopPropagation();
                                                const rows = [["اسم الطالب", `درجة (${q.title})`]];
                                                filteredStudents.forEach((student) => {
                                                  const g = (student.detailedGrades || []).find((grade) => grade.quizTitle === q.title);
                                                  rows.push([student.name, g ? String(g.score) : "-"]);
                                                });
                                                ...
                                                triggerToast(`📋 تم نسخ درجات اختبار (${q.title}) لجميع الطلاب (${filteredStudents.length} طالب) للصق المباشر في Excel!`, "success");
                                              }}
                                              className="inline-flex items-center gap-1 px-1.5 py-0.5 text-[9px] font-extrabold text-emerald-800 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200/80 rounded-md transition-all cursor-pointer shadow-3xs active:scale-95"
                                              title="نسخ أسماء ودرجات هذا الاختبار لجميع الطلاب للصقها المباشر في ملف Excel (Ctrl+V)"
                                            >
                                              <Copy className="w-2.5 h-2.5 text-emerald-600 shrink-0" />
                                              <span>نسخ للإكسل</span>
                                            </button>
                                              ...
                                              onClick={(e) => {
                                                ...
                                                if (selectedIds.length > 0) {
                                                  triggerConfirm(
                                                    ...
                                                    `هل أنت تأكد من حذف نتيجة اختبار "${q.title}" للطلاب المحددين (${selectedIds.length} طالب) لإتاحة إعادة الاختبار لهم؟`,
                                                    () => handleResetQuizForStudents(selectedIds, q.title),
                                                    undefined,
                                                    "نعم، حذف النتيجة والإعادة",
                                                    "إلغاء"
                                                  );
                                                } else {
                                                  const studentsWithQuiz = filteredStudents.filter((s) =>
                                                    (s.detailedGrades || []).some(
                                                      (g) => g.quizTitle === q.title
                                                    )
                                                  );
                                                  if (studentsWithQuiz.length === 0) {
                                                    triggerToast(
                                                      "لا يوجد طلاب لديهم نتائج لهذا الاختبار في الكشف الحالي",
                                                      "info"
                                                    );
                                                    return;
                                                  }
                                                  triggerConfirm(
                                                    "حذف نتيجة الاختبار للجميع",
                                                    `لم تقم بتحديد طلاب بأسهم التحديد.\n\nهل ترغب في حذف نتيجة اختبار "${q.title}" لجميع طلاب هذا الفصل المتقدمين (${studentsWithQuiz.length} طالب) لإتاحة إعادة الاختبار لهم؟`,
                                                    () => handleResetQuizForStudents(
                                                      studentsWithQuiz.map((s) => s.id),
                                                      q.title
                                                    ),
                                                    undefined,
                                                    "نعم، حذف للجميع للإعادة",
                                                    "إلغاء"
                                                  );
                                                }
                                              }}
                                              className="inline-flex items-center gap-1 px-1.5 py-0.5 text-[9px] font-extrabold text-rose-700 bg-rose-50 hover:bg-rose-100 border border-rose-200/80 rounded-md transition-all cursor-pointer shadow-3xs active:scale-95"
                                              title={
                                                selectedCount > 0
                                                  ? `حذف نتيجة هذا الاختبار للطلاب المحددين (${selectedCount}) لإعادة الاختبار`
                                                  : "حذف نتيجة هذا الاختبار لجميع طلاب الفصل لإعادة الاختبار"
                                              }
                                            >
                                              <RotateCcw className="w-2.5 h-2.5 text-rose-600 shrink-0" />
                                              <span>{selectedCount > 0 ? `حذف للمحددين (${selectedCount})` : "حذف النتيجة لإتاحة إعادة الاختبار"}</span>
                                            </button>
                                          </div>
                                        </th>
                                      );
                              ...
                              <tbody>
                                {filteredStudents.map((student, idx) => (
                                  ...
                                  >
                                    ...
                                    <td className="py-1.5 px-2 border border-slate-200 text-center font-mono font-bold text-slate-400 bg-slate-50 w-12 select-none">
                                      {activeTab === "student_results" ? (
                                        <div className="flex items-center justify-center gap-1.5">
                                          <input
                                            type="checkbox"
                                            className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer w-3.5 h-3.5"
                                            checked={!!selectedDeleteStudentIds[student.id]}
                                            onChange={(e) => {
                                              setSelectedDeleteStudentIds((prev) => ({
                                                ...prev,
                                            ...
                                            title="تحديد الطالب لإلغاء النتائج أو الحذف"
                                          />
                                    ...
                                    {activeTab === "students" ? (
                                      ...
                                      <>
                                        {/* Quizzes cells specifying dynamic grades with delete button */}
                                        {activeClassQuizzes.map((q) => {
                                          const gradeObj = (
                                          ...
                                          return (
                                            <td
                                              key={q.id}
                                              className="py-1.5 px-2 border border-slate-200 text-center font-sans font-extrabold text-xs text-indigo-850 bg-white hover:bg-indigo-50/20 transition-colors"
                                            >
                                              {gradeObj ? (
                                                  ...
                                                  <button
                                                    ...
                                                    onClick={(e) => {
                                                      ...
                                                      triggerConfirm(
                                                        ...
                                                        `هل تريد حذف نتيجة اختبار "${q.title}" للطالب "${student.name}" (الدرجة الحالية: ${gradeObj.score}) لإتاحة إعادة الاختبار له؟`,
                                                        () => handleResetQuizForStudents([student.id], q.title),
                                                        undefined,
                                                        "نعم، حذف النتيجة للإعادة",
                                                        "إلغاء"
                                                      );
                                                    }}
                                                    className="p-1 rounded-md text-rose-500 hover:text-rose-700 hover:bg-rose-50 border border-transparent hover:border-rose-200 transition-all cursor-pointer opacity-70 group-hover:opacity-100"
                                                    title={`حذف نتيجة "${q.title}" للطالب (${student.name}) لإعادة الاختبار`}
                                                  >
              ...
              return (
                <div className="fixed inset-0 bg-slate-900/60 z-[60] backdrop-blur-xs flex items-center justify-center p-4">
                  ...
                  >
                    ...
                    <div className="p-6 space-y-6">
                      ...
                      <div className={`space-y-3 border rounded-2xl p-4 transition-all ${
                        ...
                        <div className="flex items-center gap-2 pb-2 border-b border-slate-200/60">
                          <Users className={`w-4 h-4 ${isBuilderTargetAudienceEnabled ? "text-indigo-600" : "text-slate-400"}`} />
                          <div>
                        ...
                        <label className={`flex items-center gap-3 cursor-pointer select-none p-3 rounded-xl border transition-all ${
                          ...
                          <input
                            type="checkbox"
                            checked={isBuilderTargetAudienceEnabled}
                            onChange={(e) => setIsBuilderTargetAudienceEnabled(e.target.checked)}
                        ...
                        <div className={`grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-1 transition-all ${!isBuilderTargetAudienceEnabled ? "opacity-50 pointer-events-none" : ""}`}>
                          {/* Target Grade Selector */}
                          <div className="space-y-1.5 text-right">
                            <label htmlFor="builder-target-grade-select" className="text-[11px] font-bold text-slate-600 block">
                              الصف الدراسي المستهدف:
                            </label>
                            <select
                              id="builder-target-grade-select"
                              disabled={!isBuilderTargetAudienceEnabled}
                              value={builderTargetGrade}
                              onChange={(e) => setBuilderTargetGrade(e.target.value)}
                            ...
                            >
                              <option value="all">جميع الصفوف (عام)</option>
                              {builderTeacherGrades.map((grd) => (
                                <option key={`bdr-grd-${grd}`} value={grd}>
                                  {grd}
                                </option>
                              ))}
                            </select>
                          ...
                          <div className="space-y-1.5 text-right">
                            <label htmlFor="builder-target-semester-select" className="text-[11px] font-bold text-slate-600 block">
                              الفصل الدراسي / الشعبة المستهدفة:
                            </label>
                            <select
                              id="builder-target-semester-select"
                              disabled={!isBuilderTargetAudienceEnabled}
                              value={builderTargetSemester}
                              onChange={(e) => setBuilderTargetSemester(e.target.value)}
                              className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-2xs disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed"
                            >
                              <option value="all">جميع الفصول والشعب (عام)</option>
                              {builderTeacherSemesters.map((sem) => (
                                <option key={`bdr-sem-${sem}`} value={sem}>
                                  {sem}
                                </option>
                              ))}
                            </select>
                    ...
                    {/* Footer */}
                    <div className="p-6 border-t border-slate-100 bg-slate-50/30 flex items-center justify-between gap-2 shrink-0">
                      {!isBuilderTargetAudienceEnabled ? (
                        <span className="text-[11px] font-bold text-amber-700 bg-amber-50 border border-amber-200/80 px-3 py-1.5 rounded-xl">
                          ⚠️ يلزم تأشير مربع الفئة المستهدفة أعلاه لتفعيل التوليد
                        </span>
                      ) : <div />}
                      <div className="flex items-center gap-2">
                        <button
                        ...
                        <button
                          type="button"
                          disabled={!isBuilderTargetAudienceEnabled || (autoBuilderMcqCount + autoBuilderTfCount <= 0)}
                          onClick={handleConfirmBuilderAutoQuiz}
                          className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 disabled:text-slate-500 disabled:shadow-none disabled:cursor-not-allowed text-white text-xs font-extrabold shadow-md shadow-indigo-100 cursor-pointer transition-all flex items-center gap-1.5 group"
                        >
          ...
          {/* Teacher Page Footer Copyright */}
          <footer className="mt-12 pt-6 pb-4 border-t border-slate-200/60 text-center text-xs text-slate-500 font-bold flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-indigo-950 font-black whitespace-nowrap">
              <Sparkles className="w-4 h-4 text-amber-500 shrink-0" />
              <span>معلم | Teacher.AI</span>
            </div>
            <p className="text-slate-400 text-[11px] font-semibold whitespace-nowrap">
